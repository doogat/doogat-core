...
def test_fix_title_format(sample_zettel_data):
    ...
    with patch.object(
        StringOperator, "replace_abbreviations", return_value="Doctor John Doe"
    ) as mock_method:
        fix_title_format(sample_zettel_data)
        mock_method.assert_called_once_with(text="PhD. John Doe", level=0)
        assert (
            sample_zettel_data.metadata["title"] == "Doctor John Doe"
...
def test_fix_title_format_idempotence(sample_zettel_data):
    ...
    with patch.object(
        ...
        fix_title_format(
            sample_zettel_data
        )  # Second call with the already modified title
